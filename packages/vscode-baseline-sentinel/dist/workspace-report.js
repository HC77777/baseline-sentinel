...
async function generateWorkspaceReport() {
    try {
        ...
        if (results.totalIssues === 0) {
            vscode.window.showInformationMessage(`✅ Workspace scan complete! No compatibility issues found in ${results.totalFiles} files.`, { modal: true });
            return;
        }
        // Get OpenAI settings
        ...
        const action = await vscode.window.showInformationMessage(`✅ Report generated! Found ${results.totalIssues} issues in ${results.fileReports.length} files.`, { modal: true }, 'Open Report', 'Copy to Clipboard');
