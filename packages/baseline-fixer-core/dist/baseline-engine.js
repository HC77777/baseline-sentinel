...
exports.isFeatureBaseline = isFeatureBaseline;
exports.isJsPropertyBaseline = isJsPropertyBaseline;
exports.isCssPropertyBaseline = isCssPropertyBaseline;
 ...
 */
function findBcdKeyForJsProperty(propertyName) {
    if (bcdKeyCache.has(propertyName)) {
        return bcdKeyCache.get(propertyName);
    }
    for (const feature of Object.values(web_features_1.features)) {
    ...
    bcdKeyCache.set(propertyName, null);
    return null;
}
const cssPropertyToFeatureIdCache = new Map();
function findFeatureIdForCssProperty(propertyName) {
    const cached = cssPropertyToFeatureIdCache.get(propertyName);
    if (cached !== undefined) {
        return cached;
    }
    for (const [featureId, feature] of Object.entries(web_features_1.features)) {
 ...
 */
function isFeatureBaseline(bcdKey) {
    if (!bcdKey) {
    ...
    try {
        ...
        const status = (0, compute_baseline_1.getStatus)(null, bcdKey);
 ...
 * @param propertyName The name of the JS property (e.g., 'keyCode').
 */
function isJsPropertyBaseline(propertyName) {
